...
$DevJunk = 'dumps', 'dump.txt', 'probe.txt', 'dev_probe.txt', 'config.txt',
           ...
           'dev_charnames.lua', 'dev_questdump.lua',
...
# Release build flag: turn OFF the developer keybinds (F4/F6/F7/F9/Ctrl+F5/
# Ctrl+Shift+F5). The dev tree keeps them (build_flags.lua ships debug = true).
# F10 (read the current objective) used to be in that list and is now a SHIPPED bind —
# players had no way to re-hear the objective (2026-07-31).
Set-Content -Path (Join-Path $StageScripts 'build_flags.lua') `
    -Value "-- Generated by package.ps1 for a release build.`nreturn {`n    debug = false,`n}`n" `
    -Encoding utf8

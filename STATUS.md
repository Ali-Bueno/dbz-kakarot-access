...
| Quest objective HUD (text) | wip | `quest_objective.lua` — `Quest_Navi_C` rows `Txt_List_00`; announces on change + F10 on demand. 2026-07-15 fix: directory-mapped `{"fm","QuestNavigation"}` — the evening dump CONFIRMS detection (`Quest_Navi_C ON-SCREEN`, objective text visible in census) yet the user hears nothing → failure is INSIDE the row/text read. F10 now appends a per-row diagnostic (valid/on_screen/txt per M00..S02 + titles + active_adapter) to `dumps/dump_quest.txt` — next session starts from that file |
...
| Soul Emblems grid / Community | wip | `screen_community.lua`. 2026-07-15 fix: `AT_UICommunityStart` UNMAPPED from the directory (third strike) — the manager has TWO flows (`cm.UIEmbListIns` board flow vs `cm.MenuSoulEmListIns` menu flow) and the menu flow's owner (`USoulEmblemMenu`/`UMenuObjectBase`) reflects NO widget field, so the mapping asserted "absent" on the menu-opened grid. Back on scan+pad-boost — AND (evening dump) the menu-flow instance is the BLUEPRINT class **`Start_Commu_Emb_C`** (census), so FindAllOf on the native name finds nothing (the Start_Commu_Brd_C lesson): `grid_host()` now scans BOTH names. Bonus fact: each grid slot (`UAT_UIXCmnEmb_Cursor`) reflects `UnlockState` u8 @0x408. Pending in-game verify |
...
back to normal — the UE4SS.log line `subtitles option: ...` says whether the gate found the save
object and what value it read; (5) start an episode: the title card should read once; (6) open Soul
Emblems from the menu: the grid should read (evening-2 fix: `grid_host()` also scans the BP name
`Start_Commu_Emb_C`); (7) the cook-NPC entry menu ("Preparar un platillo"/"Salir") should read
(evening-2 fix: second `Shop_Top_C` chain via `CookingMenu.WL_CookingTop`); (0-bis) if the
objective HUD is still mute, ONE F10 press writes the decisive per-row diagnostic to
`dumps/dump_quest.txt` — send that file; (+) the Community-board d-pad snap feature is still
pending (needs a mem_bridge write API — design session of its own).
...
- **CODED (2026-07-15 evening): the cook-NPC ENTRY menu ("Preparar un platillo" / "Salir").**
  ...
  `{"fm","CookingMenu","WL_CookingTop"}`. Pending in-game verify. (General lesson: mapping a
  POOLED multi-instance class to ONE field silently drops the other instances — map every
  holder or don't map.)

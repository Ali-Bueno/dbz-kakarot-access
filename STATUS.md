...
| Quest objective HUD (text) | wip | `quest_objective.lua`. 2026-07-15 SOLVED in two steps: (1) directory-mapped `{"fm","QuestNavigation"}` (detection was scan-starved); (2) the F10 diagnostic (`dump_quest.txt`) proved a single-objective quest puts the text in the TITLE node (`Txt_Main00`/`WL_MainQuestListTitle`) with every M/S row hidden — the rows are only the multi-step checklist — and `objective_text()` required rows, so it always returned nil. Groups now speak on title OR rows. Diagnostic still armed (every F10 appends to dump_quest.txt); remove once verified. Pending in-game verify |
